...
namespace ppa_lab_test_1
{
    ...
    public partial class Form3 : Form
    {
        ...
        private string LivesInfo(Army army)
        {
            string res = "";
            for (int i = 0; i < army.units.Count(); i++)
            {
                res += $"{army.units[i].Name}: {army.units[i].Health} | ";
            }
            return res;
        }
        ...
        private void attackToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (fight == 1)
            {
                ...
                archer.Tick += (sender, args) =>
                {
                    ...
                    if (game.player.units.Count != 0) pBoxAP[0].Image = game.player.units[game.player.units.Count() - 1].ImgsP.StandingStill;
                    if (game.enemy.units.Count != 0) pBoxAE[0].Image = game.enemy.units[game.enemy.units.Count() - 1].ImgsE.StandingStill;
            ...
            if (fight == 2)
            {
                ...
                archer.Tick += (sender, args) =>
                {
                    ...
                    {
                        pBoxAP[0].Image = game.player.units[game.player.units.Count() - 2].ImgsP.StandingStill;
                        pBoxAP[1].Image = game.player.units[game.player.units.Count() - 1].ImgsP.StandingStill;
                    }
                    if (game.enemy.units.Count() == 1)
                    {
                    ...
                    {
                        pBoxAE[0].Image = game.enemy.units[game.enemy.units.Count() - 2].ImgsE.StandingStill;
                        pBoxAE[1].Image = game.enemy.units[game.enemy.units.Count() - 1].ImgsE.StandingStill;
                    }
                    // After 5 seconds, restore the original image
                    for (int i = 0; i < game.player.units.Count() - 2; i++)
                    {
                        if (game.player.units[i].Name.Contains("Archer"))
                        {
                            pBoxAP[i + 2].Image = game.player.units[i].ImgsP.Special;
                        }
                    }
                    for (int i = 0; i < game.enemy.units.Count() - 2; i++)
                    {
                        if (game.enemy.units[i].Name.Contains("Archer"))
                        {
                            pBoxAE[i + 2].Image = game.enemy.units[i].ImgsE.Special;
                        }
                ...
                stand.Tick += (sender, args) =>
                {
                    ...
                    for (int i = 0; i < game.player.units.Count() - 2; i++)
                    {
                        if (game.player.units[i].Name.Contains("Archer"))
                        {
                            pBoxAP[i + 2].Image = game.player.units[i].ImgsP.StandingStill;
                        }
                    ...
                    for (int i = 0; i < game.enemy.units.Count() - 2; i++)
                    {
                        if (game.enemy.units[i].Name.Contains("Archer"))
                        {
                            pBoxAE[i + 2].Image = game.enemy.units[i].ImgsE.StandingStill;
                        }
